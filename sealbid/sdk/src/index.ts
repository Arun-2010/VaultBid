...
import { IDL, Sealbid } from "../types/sealbid";
...
export class SealBidSDK {
  private program: Program<Sealbid>;
  ...
  constructor(
    ...
    this.program = new Program<Sealbid>(IDL, programId, provider);
  }
  ...
  /**
   * Slashes funds from bidders who didn't reveal their bids
   */
  async slashNonRevealer(auction: PublicKey, bidder: PublicKey): Promise<string> {
    const auctionInfo = await this.getAuctionInfo(auction);
  ...
  /**
   * Gets all auctions for a given authority
   */
  async getAuctionsByAuthority(authority: PublicKey): Promise<PublicKey[]> {
    // This would require implementing a more sophisticated filtering mechanism
